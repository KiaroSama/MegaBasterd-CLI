...
@click.pass_context
def split_cmd(ctx: click.Context, source: Path, part_size_mb: int, output_dir: Path | None) -> None:
...
def merge_cmd(
    ...
    try:
        ...
        # Must be non-zero: a refused merge (alias guard) followed by
        # `&& rm parts.*` would otherwise delete what the guard protected.
        print_error(str(exc))
        ctx.exit(1)
...
@click.pass_context
def thumbnail_cmd(ctx: click.Context, source: Path, destination: Path) -> None:
    """Generate a thumbnail (max 250x250) suitable for MEGA file previews."""
    if not create_thumbnail(source, destination):
        print_error(
