...
# The native Windows secure-open helper. Resolved from the launcher's own
# location, so it is found however the script is invoked and from any cwd.
$script:SecureLogSourcePath = Join-Path $ProjectRoot "launcher/SecureLog.cs"
...
function Initialize-SecureLogType {
    ...
    # 5.1, both one-time.
    #
    # The C# lives in launcher/SecureLog.cs rather than in a here-string here,
    # because 350 lines of it made this file the thing it is supposed not to be.
    # A missing or unreadable file throws, which the caller turns into "file
    # logging disabled" - there is deliberately no managed fallback, since
    # falling back is exactly how the create-time DACL window existed.
    if ($script:SecureLogTypeReady) {
        return
    }
    Add-Type -Path $script:SecureLogSourcePath
