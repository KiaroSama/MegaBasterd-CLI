...
### Fixed
- Adding an account to a vault that cannot be opened now says why. Every failure was reported as "passphrase does not match", so anyone holding credentials written before the vault format change was told their passphrase was wrong and retyped it repeatedly - no passphrase opens an older-format credential, and the vault already said exactly that before the message was discarded. A genuinely wrong passphrase still reports as one. (`mb account remove` needs no passphrase, so the advice it gives always works.)
