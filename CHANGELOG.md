...
- Versioned upload resume identity: path, size, `mtime_ns`, platform file id, and a multi-region sampled content fingerprint. A same-size replaced or modified file can no longer reuse stale uploaded chunks; the identity is re-checked before finalization and mutation during transfer aborts safely. Legacy states without an identity restart fresh.
...
- Centralized config validation for `config set` and hand-edited files (ports, timeouts, worker counts, speed limits, quota waits, log settings) with safe fallbacks and warnings.
